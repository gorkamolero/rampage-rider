...
export class TableManager {
  ...
  // Shared shadow manager for patrons
  ...
  constructor(scene: THREE.Scene) {
    ...
    // Shadow manager for table patrons (max 16: 4 tables * 4 patrons)
    this.shadowManager = new InstancedBlobShadows(scene, 16);
  ...
  private createPatron(position: THREE.Vector3, facingAngle: number): TablePatron | null {
    ...
    // Reserve shadow
    const shadowIndex = this.shadowManager.reserveIndex();
    if (shadowIndex >= 0) {
      this.shadowManager.updateShadow(shadowIndex, position.x, position.z, 0.6);
    }
  ...
  private disposePatron(patron: TablePatron): void {
    ...
    if (patron.shadowIndex >= 0) {
      this.shadowManager.releaseIndex(patron.shadowIndex);
    }
